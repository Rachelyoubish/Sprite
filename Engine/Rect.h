...
public:
	...
	Rect_(const Vec2_& topLeft, const Vec2_& bottomRight)
		:
		Rect_(topLeft.x, bottomRight.x, topLeft.y, bottomRight.y)
	{}

	Rect_(const Vec2_& topLeft, T width, T height)
		:
		Rect_(topLeft, topLeft + Vec2_(width, height))
	{}
	...
	Rect_ FromCenter(const Vec2_& center, T halfWidth, T halfHeight) {
	...
	Vec2_ GetCenter() const {
		return Vec2_((left + right) / 2, (top + bottom) / 2);
	}
